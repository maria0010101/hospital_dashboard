...
提供一份已全面混淆的業務資料樣本檔，供功能展示與格式參考（不含任何真實院區、單位、人員與數值）：

- **樣本檔**：`sample_data/sample_data-1150826.xlsx`（9 工作表、約 7.7 萬列；檔名採 ASCII 以利各平台流通）
- **產製工具**：`sample_data/make_sample.py`（自原始檔擷取 3 個院區後混淆，可重現）
- 混淆方式為**全域一對一固定對應**：同一真實名稱 → 同一假名，跨工作表一致
...
- 數值採「院區固定倍率縮放」（×0.72／×0.64／×0.86，四捨五入）；**佔床率等比率欄位與年月／當月日數等曆法欄位維持原值**，故佔床率、增減率、健保／自費佔比等報表指標皆與原始資料一致，僅絕對數值不同
- 病床服務量（病床別業務資料）的 護理站／大類別／病床類別 一併混淆，「佔床率報表護理站／病床類別」欄位與合併鍵亦同步以假名重建
- 混淆對照表（真實 → 假名）僅存於產製端本機，**不隨 repo 發布**
...
**② 病床利用分頁** — 病床類別篩選為病床類別01..13，趨勢圖依混淆類別繪製
...
- **樣本資料（2026-08-28 更新）**：病床服務量（病床別業務資料）新增 護理站／大類別／病床類別 混淆，佔床率報表欄位與合併鍵同步重建；樣本檔與產製工具置於 `sample_data/`（詳見「樣本資料」一節）
